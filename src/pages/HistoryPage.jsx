...
function PlayerAvatar({ imagePath, name, size = "md" }) {
  ...
  return (
    <div className={`hp-avatar hp-avatar--${size}`}>
      {url ? <img src={url} alt={name} className="hp-avatar-img" /> : <span className="hp-avatar-initials">{initials}</span>}
    </div>
  );
}
...
function PlayerDetail({ playerId, onBack }) {
  ...
  if (loading) return <div className="hp-detail-loading"><RefreshCw size={20} className="hp-spin" /><span>Cargando jugador...</span></div>;
  ...
  const lifespan = player.birth_year ? `${player.birth_year}${player.death_year ? ` – ${player.death_year}` : " – presente"}` : null;

  return (
    <div className="hp-detail">
      <button className="hp-detail-back" onClick={onBack}><ArrowLeft size={14} /><span>Volver a jugadores</span></button>
      <div className="hp-detail-hero">
        <div className="hp-detail-hero-img-wrap">
          {imageUrl ? <img src={imageUrl} alt={player.name} className="hp-detail-hero-img" />
            : <div className="hp-detail-hero-placeholder">{player.name.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase()}</div>}
              ...
              return (
                <div key={i} className="hp-team-row">
                  <div className="hp-team-logo-wrap">
                    {teamImg ? <img src={teamImg} alt={team?.name} className="hp-team-logo" />
...
export default function HistoryPage() {
  const [activeSection, setActiveSection] = useState("players"); // "players" | "teams"
        ...
        {showFilters && (
          <div className="hp-filters-panel">
            <div className="hp-filters-row">
              ...
              {hasActiveFilters && <button className="hp-clear-filters" onClick={clearFilters}><X size={11} /> Limpiar</button>}
            </div>
          </div>
        )}

        {loading && <div className="hp-state-msg"><RefreshCw size={18} className="hp-spin" /><span>Cargando leyendas...</span></div>}
        {error && !loading && <div className="hp-state-error"><AlertCircle size={18} /><p>{error}</p><button className="hp-retry-btn" onClick={reload}><RefreshCw size={12} /> Reintentar</button></div>}
        ...
        {!loading && !error && players.length > 0 && (
          <>
            <div className="hp-results-bar">
              <span className="hp-results-count">{players.length} jugador{players.length !== 1 ? "es" : ""}{hasActiveFilters || search ? " encontrado" + (players.length !== 1 ? "s" : "") : ""}</span>
            </div>
            <div className="hp-grid">
              {players.map((player) => (
                <PlayerCard key={player.id} player={player} isActive={player.id === selectedPlayerId}
